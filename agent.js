...
async function extractInteractiveElements(page) {
  try {
    return await page.evaluate(() => {
      ...
      // ⚡ Bolt: Fast live HTMLCollection extraction without Array.from() overhead
...
export async function extractInternalLinks(startUrl) {
  ...
  try {
    ...
    const hrefs = await page.evaluate(() => {
      // ⚡ Bolt: Fast DOM element retrieval avoiding CSS parsing and Array.from
      ...
      try {
        performanceMetrics = await page.evaluate(() => {
          ...
          return {
            ...
            // ⚡ Bolt: Fast DOM element retrieval
