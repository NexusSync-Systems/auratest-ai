...
async function extractInteractiveElements(page) {
  try {
    return await page.evaluate(() => {
      ...
      // ⚡ Bolt: Nahrazeno pomalé querySelectorAll a Array.from rychlejším getElementsByTagName a pre-alokovaným polem
...
export async function extractInternalLinks(startUrl) {
  ...
  try {
    ...
    const hrefs = await page.evaluate(() => {
      // ⚡ Bolt: Nahrazeno pomalé querySelectorAll('a') a Array.from rychlejším getElementsByTagName
      const anchorCollection = document.getElementsByTagName('a');
      const len = anchorCollection.length;
      const urls = new Array(len);
      for (let i = 0; i < len; i++) {
        urls[i] = anchorCollection[i].href;
      }
      return urls;
    });
