...
async function extractInteractiveElements(page) {
  try {
    return await page.evaluate(() => {
      ...
      // ⚡ Bolt: Fast DOM retrieval using getElementsByTagName instead of querySelectorAll
      // Avoiding Array.from allocation overhead by pre-allocating an array and using a for-loop
...
export async function extractInternalLinks(startUrl) {
  ...
  try {
    ...
    const hrefs = await page.evaluate(() => {
      // ⚡ Bolt: Use getElementsByTagName instead of querySelectorAll for faster DOM access
