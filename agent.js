...
async function extractInteractiveElements(page) {
  try {
    return await page.evaluate(() => {
      ...
      // ⚡ Bolt: Použití getElementsByTagName('*') místo querySelectorAll('*') a alokace přes rychlý cyklus
...
export async function extractInternalLinks(startUrl) {
  ...
  try {
    ...
    const hrefs = await page.evaluate(() => {
      // ⚡ Bolt: Optimalizace výběru a alokace odkazů pomocí getElementsByTagName
      const linksCollection = document.getElementsByTagName('a');
      const len = linksCollection.length;
      const hrefsArray = new Array(len);
      for (let i = 0; i < len; i++) {
        hrefsArray[i] = linksCollection[i].href;
      }
      return hrefsArray;
    });
