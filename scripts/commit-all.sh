#!/usr/bin/env bash
#
# Provede všech osm commitů z COMMITS.md v jednom běhu.
# Vygenerováno ze samotného COMMITS.md, takže zprávy zůstávají v souladu.
#
...
if [ -n "$(git status --porcelain 2>/dev/null | head -1)" ]; then
  echo "Repozitář má změny — commituju je podle COMMITS.md."
else
  echo "Není co commitovat."
  exit 0
fi

# Zbytky po přerušeném commitu. Kromě HEAD.lock a index.lock vzniká i zámek
# na konkrétní větev (.git/refs/heads/<vetev>.lock) — ten se snadno přehlédne.
#
# Mažeme jen když neběží žádný git proces, aby se neshodil legitimní commit
# spuštěný odjinud (editor, IDE).
if pgrep -x git >/dev/null 2>&1; then
  echo "Běží jiný git proces — ukončete ho a spusťte skript znovu." >&2
  exit 1
fi

stale_locks=$(find .git -name '*.lock' 2>/dev/null || true)
if [ -n "$stale_locks" ]; then
  echo "Odstraňuji zbylé zámky:"
...
git rm -r --cached frontend/dist -q 2>/dev/null || true  # už nemusí být sledovaný
...
echo ""
echo "─── Commit 3 — validita compliance skenerů ───"
git add agent.js
git commit --allow-empty -F- <<'EOF'
...
echo ""
echo "─── Commit 5 — frontend a testy ───"
...
echo ""
echo "─── Commit 6 — frontend testy, přístupnost a rozdělení App.jsx ───"
...
echo ""
echo "─── Commit 7 — regrese z předchozích commitů a dosud nereviewované soubory ───"
...
echo ""
echo "─── Commit 8 — zbylé nálezy a nesoulad dokumentace ───"
